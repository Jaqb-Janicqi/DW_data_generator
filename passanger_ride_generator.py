...
class PassangerGenerator:
    sections = [1,3,2,1,3,1,2,2,1,3,1,2,3,1,3,1] # station0->1, station1->2
    ...
    def __init__(self, year):
        ...
        self.dates = [date for month in self.calendar_tuples for week in month for day in week for date in day]
    ...
    def generate_passanger_rides(self):
        for date in self.dates:
            ...
            for minute_floor in range(1440):
                minute_ceil = minute_floor + 1 # distribution is in hours, we are interested in 1 minute intervals
    ...
    def generate_ride(self, date, num_passangers, hour, minute): # załadować excela, zobaczyć najbliższy pociąg i dopiero liczyć czas przejazdu
        for i in range(num_passangers):
            exit_section_id = entry_section_id = np.random.randint(1, self.num_sections + 1)
            ...
            if entry_section_id < exit_section_id:
                travel_time = sum(self.sections[entry_section_id:exit_section_id])
            else:
                travel_time = sum(self.sections[exit_section_id:entry_section_id])
            entry_time = calendar.datetime.datetime(date.year, date.month, date.day, hour, minute, np.random.randint(0, 60))
            exit_time = entry_time + calendar.datetime.timedelta(minutes=travel_time)
            ...
            passenger_ride = PassangerRide(entry_section_id, exit_section_id, self.id, entry_time, exit_time)
